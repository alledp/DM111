...
public class UserService {
    ...
    private User buildUser(UserRequest request) {
        ...
        return new User(userId, request.name(), request.email(), encryptedPwd, request.category(), User.UserType.valueOf(request.type()));
    }

    private User buildUser(UserRequest request, String id) {
        var encryptedPwd = encryptor.encrypt(request.password());
        return new User(id, request.name(), request.email(), encryptedPwd, request.category(), User.UserType.valueOf(request.type()));
    }

    private UserResponse buildUserResponse(User user) {
        return new UserResponse(user.id(), user.name(), user.email(), user.category(), user.type().name());
    }
