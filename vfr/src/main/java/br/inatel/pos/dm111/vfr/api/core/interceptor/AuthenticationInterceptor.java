...
import br.inatel.pos.dm111.vfr.persistence.promotion.FirebasePromotionRepositoryImpl;
...
public class AuthenticationInterceptor implements HandlerInterceptor {
    ...
    public AuthenticationInterceptor(JwtParser jwtParser, UserRepository userRepository, RestaurantRepository restaurantRepository, PromotionRepository promotionRepository) {
    ...
    private void authenticateRequest(AppJwtToken appJwtToken) throws ApiException {
        ...
        if (appJwtToken.uri().startsWith("/valefood/promotion/")) {
            if (appJwtToken.method().equals(HttpMethod.PUT.name()) ||
                    appJwtToken.method().equals(HttpMethod.DELETE.name()))  {
                var splitUri = appJwtToken.uri().split("/");
                var pathPromotionId = splitUri[3];
                var promotion = retrievePromotionById(pathPromotionId).orElseThrow(() -> {
